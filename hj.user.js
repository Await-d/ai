...
// @downloadURL https://update.greasyfork.org/scripts/485701/%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/485701/%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA.meta.js
// ==/UserScript==
...
(function () {
  ...
  if (isphone) {
    $("body").append(
      " <div id='dmyDiv'; style='right: 3px;width: 40px;height: 40px;font-size: 12px;border-radius:50%; bottom: 160px;background: #FF6666;color:#ffffff;overflow: hidden;z-index: 9999;position: fixed;padding:5px;text-align:center;'>下载</div>"
    );
    $("body").append(
      " <div id='jxmyDiv'; style='right: 3px;width: 40px;height: 40px;font-size: 12px;border-radius:50%; bottom: 200px;background: #FF6666;color:#ffffff;overflow: hidden;z-index: 9999;position: fixed;padding:5px;text-align:center;'>等待解析</div>"
    );
    $("body").append(
      " <div id='jxspmyDiv'; style='right: 3px;width: 40px;height: 40px;font-size: 12px;border-radius:50%; bottom: 240px;background: #FF6666;color:#ffffff;overflow: hidden;z-index: 9999;position: fixed;padding:5px;text-align:center;'>准备中</div>"
    );
  } else {
    $("body").append(
      " <div id='dmyDiv'; style='right: 3px;width: 50px;height: 50px;font-size: 16px;border-radius:50%; bottom: 170px;background: #FF6666;color:#ffffff;overflow: hidden;z-index: 9999;position: fixed;padding:5px;text-align:center;'>下载</div>"
    );
    $("body").append(
      " <div id='jxmyDiv'; style='right: 3px;width: 50px;height: 50px;font-size: 16px;border-radius:50%; bottom: 220px;background: #FF6666;color:#ffffff;overflow: hidden;z-index: 9999;position: fixed;padding:5px;text-align:center;'>等待解析</div>"
    );
    $("body").append(
      " <div id='jxspmyDiv'; style='right: 3px;width: 50px;height: 50px;font-size: 16px;border-radius:50%; bottom: 270px;background: #FF6666;color:#ffffff;overflow: hidden;z-index: 9999;position: fixed;padding:5px;text-align:center;'>准备中</div>"
    );
  }

  var jxspmyDiv = document.getElementById("jxspmyDiv");
  jxspmyDiv.addEventListener("click", function (e) {
    if (
      fasongtext.indexOf("IV=") === -1 ||
      fasongtext.indexOf("包含") === -1 ||
      fasongtext.indexOf("[--秒]") > -1
    ) {
  ...
  var jxmyDiv = document.getElementById("jxmyDiv");
  jxmyDiv.addEventListener("click", function (e) {
    const textarea = document.createElement("textarea");
    if (downloadurl === "") {
      jxmyDiv.innerHTML = "请先解析视频";
      return;
    }
    ...
    jxmyDiv.innerHTML = "复制下载链接成功";
    //jxmyDiv.innerHTML = "fff"
  });

  var dmyDiv = document.getElementById("dmyDiv");
  dmyDiv.addEventListener("click", function (e) {
    ...
    let url2 =
      "&referer=" +
      currentUrl +
      "&filename=%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA";
    ...
    if (isphone) {
      ...
      var intervalId = setInterval(function () {
        ...
        if (infophone) {
          ...
          if (
            textsent.indexOf("IV=") === -1 ||
            textsent.indexOf("包含") === -1 ||
            textsent.indexOf("[--秒]") > -1
          ) {
            jxspmyDiv.innerHTML = "解析失败";
            alert("准备失败，请刷新重试");
            return;
  ...
  function fasong(textsent) {
    runAsync("http://ip.haijiao007.asia/getnmm1", "POST", textsent)
      //   .then((result) => {
      //     return result;
      //   })
      .then((result) => {
  ...
  function replace_exist_img(body) {
    ...
    let re_img =
      /<img src=\"https:\/\/[\w\.\/]+?\/images\/.*?\" data-id=\"(\d+)\".*?\/>/g;
  ...
  function modify_data(data) {
    ...
    if ("sale" in body && body.sale !== null) {
      ...
      body.sale.buy_index = parseInt(
        Math.random() * (5000 - 1000 + 1) + 1000,
        10
      );
    }
    if (has_video >= 0) {
    ...
    let ncontent = body.content.replace(
      /<span class=\"sell-btn\".*<\/span>/,
      selled_img
    );
      ...
      if (!clicked_flag) {
        for (const element of document.getElementsByClassName(
          ...
          if (element.innerText.indexOf("令牌已过期") > -1) {
            clicked_flag = true;
            let e = element.querySelector(
              "div.el-message-box__header > button"
            );
            setTimeout(
              (e) => {
                e.click();
